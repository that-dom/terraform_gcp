...
# ssl証明証の登録
module "ssl_certificate_web" {
  source = "../../modules/ssl_certificate"

  ssl_certificate_variables {
    name        = "web-ssl"
    description = "${var.ssl_certificate_web_settings["description"]}"
    private_key = "${var.ssl_certificate_web_settings["private_key"]}"
    certificate = "${var.ssl_certificate_web_settings["certificate"]}"
    chain_key   = "${var.ssl_certificate_web_settings["chain_key"]}"
  }
}

# target_https_proxy
module "target_https_proxy_web" {
...
module "global_forwarding_rule_web" {
  ...
  global_forwarding_rule_variables {
    name       = "web-fr"
    target     = "${module.target_https_proxy_web.https_proxy_link}"
    port_range = "443"
